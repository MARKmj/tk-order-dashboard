...
<body class="locked">
  ...
  <script>
    ...
    function renderDetails(rows) {
      ...
      const trs = recent.map(r => '<tr><td>' + esc(r.orderDate) + '</td><td>' + esc(r.site) + '</td><td>' + esc(r.account) + '</td><td><div class="name" title="' + esc(r.productName) + '">' + esc(r.productName) + '</div></td><td class="num">' + money(r.orderAmount) + '</td><td class="num">' + money(r.actualCommissionCny) + '</td><td class="num">' + esc(r.orderId) + '</td></tr>').join("");
      document.getElementById("detailTable").innerHTML = '<table><thead><tr><th>日期</th><th>站点</th><th>达人</th><th>商品</th><th class="num">GMV</th><th class="num">佣金</th><th class="num">订单号</th></tr></thead><tbody>' + trs + '</tbody></table>';
    }
