...
async function listRecords(project) {
  ...
  do {
    ...
    url.searchParams.set("view_id", project.viewId);
...
function buildPayload(sourceRows, project) {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .auth-brand { color: var(--blue); font-size: 13px; font-weight: 760; margin-bottom: 8px; }
    .auth-card h2 { font-size: 25px; margin-bottom: 8px; }
    .auth-card p { margin: 0; color: var(--muted); font-size: 13px; line-height: 1.65; }
    .project-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; margin: 20px 0 16px; }
    .project-card {
      display: block;
      width: 100%;
      min-height: 120px;
...
<body class="locked">
  <section class="auth-overlay" id="authOverlay" aria-modal="true" role="dialog">
    <div class="auth-card">
      <div class="auth-brand">贯一恒新科技</div>
      <h2>TK矩阵项目数据中心</h2>
      <p>围绕 TikTok 矩阵账号运营、短视频商品内容分发与带货效果追踪，构建面向跨境商品的订单与佣金数据看板，用于观察项目增长、达人效率、商品趋势和市场结构。</p>
...
async function loadPayload(projectId = "main") {
  const project = getProject(projectId);
  return buildPayload(await listRecords(project), project);
}
