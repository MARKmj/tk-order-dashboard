  ...
  <script>
    ...
    function filteredRows(includeEntityFilter = true) {
      ...
      return payload.rawRows.filter(row => {
        ...
        if (!q) return true;
        return [row.team, row.site, row.account, row.orderId, row.productId, row.productName, row.productNameCn].some(v => String(v || "").toLowerCase().includes(q));
      });
    }
    ...
    function exportRank(kind) {
      ...
      const bodyRows = data.map(d => [
        ...
        ...(isAccount ? [] : [productMetaFromGroup(d)]),
      ...
      downloadExcel(title, isAccount ? headers : ["商品名称-中文", "商品ID / 原商品名", ...headers.slice(1)], bodyRows);
    }
