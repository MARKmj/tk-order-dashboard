...
const { enrichProductTranslations } = require("./product-translations");
...
const PAGE_SIZE = 500;
const FEISHU_PAGE_TIMEOUT_MS = Number(process.env.FEISHU_PAGE_TIMEOUT_MS || 45000);
const FEISHU_PAGE_MAX_ATTEMPTS = Number(process.env.FEISHU_PAGE_MAX_ATTEMPTS || 3);
...
async function feishuJson(url, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      lastError = error.name === "AbortError" ? new Error("Feishu request timeout, please try again later") : error;
...
function isDataNotReady(error) {
  return /Data not ready|try again later|timeout/i.test(error?.message || "");
}
...
async function listRecordsPage(project, token, pageToken, useView) {
  ...
  return feishuJson(url, {
    ...
    maxAttempts: FEISHU_PAGE_MAX_ATTEMPTS,
    timeoutMs: FEISHU_PAGE_TIMEOUT_MS,
  });
}
...
function parseDate(value) {
  ...
  if (typeof value === "string" && /^\d{11,}$/.test(value.trim())) return new Date(Number(value.trim()));
  const localMatch = String(value || "").trim().match(/^(\d{4})[-/](\d{1,2})[-/](\d{1,2})(?:[ T](\d{1,2}):(\d{1,2})(?::(\d{1,2}))?)?$/);
  if (localMatch) {
...
function parseHour(value, fallbackTime) {
  ...
  return date ? Number(timeZoneParts(date).hour) : null;
}
...
function buildPayloadFromRows(rows, project, meta = {}) {
...
async function buildPayload(sourceRows, project, meta = {}) {
  const rows = normalizeRows(sourceRows);
  const translations = await enrichProductTranslations(rows);
  return buildPayloadFromRows(rows, project, { ...meta, translations });
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .rank-scroll th:nth-child(2), .rank-scroll td:nth-child(2) { width: 38%; }
    .rank-scroll th.num, .rank-scroll td.num { width: 14%; }
  ...
  <main class="shell">
    <header>
      ...
      <div class="toolbar">
        ...
        <button id="refreshBtn" type="button">刷新看板</button>
        <button class="primary" id="syncBtn" type="button">同步飞书</button>
        <span class="status" id="refreshStatus">读取服务器快照</span>
      </div>
    </header>
  ...
  <script>
    ...
    function filteredRows(includeEntityFilter = true) {
      ...
      return payload.rawRows.filter(row => {
        ...
        if (!q) return true;
        return [row.team, row.site, row.account, row.orderId, row.productId, row.productName, row.productNameCn, row.productCategory, (row.productSellingPoints || []).join(" ")].some(v => String(v || "").toLowerCase().includes(q));
      });
    }
    ...
    function productKey(row) {
      return row.productId || row.productNameCn || row.productName || "未填写";
    }

    function productTitleFromGroup(group) {
      const item = group?.items?.[0] || {};
      return item.productNameCn || item.productName || item.productId || group?.key || "-";
    }

    function productMetaFromGroup(group) {
      const item = group?.items?.[0] || {};
      const parts = [];
      if (item.productCategory) parts.push(item.productCategory);
      if (Array.isArray(item.productSellingPoints) && item.productSellingPoints.length) parts.push(item.productSellingPoints.join("、"));
      const original = productOriginalMetaFromGroup(group);
      if (original) parts.push(original);
      return parts.join("｜");
    }

    function productOriginalMetaFromGroup(group) {
      const item = group?.items?.[0] || {};
      const parts = [];
      if (item.productId) parts.push("ID " + item.productId);
      if (item.productName && item.productName !== item.productNameCn) parts.push(item.productName);
      return parts.join("｜");
    }
    ...
    function updateTrendOptions() {
      ...
      let products = rankData(rows, productKey, "commission", "all").map(d => d.key);
      const productQuery = state.productIdSearch.trim();
      if (productQuery) {
        products = products.filter(key => key.includes(productQuery)).slice(0, 200);
      } else {
      ...
      fillSelect("productTrendSelect", products.map(key => {
        const group = rankData(rows.filter(r => productKey(r) === key), productKey, "commission", 1)[0];
        return { value: key, label: productTitleFromGroup(group) + (group?.key ? "｜" + group.key : "") };
      }), productQuery ? "无匹配商品" : "选择商品");
    ...
    function renderInsights(rows) {
      ...
      const products = rankData(rows, productKey, "commission", 5);
      ...
      document.getElementById("insights").innerHTML = [
        ...
        ["头部商品", productTitleFromGroup(products[0]).slice(0, 56) + "，贡献 GMV " + money(products[0]?.orderAmount || 0) + "，佣金 " + money(products[0]?.commission || 0) + "。"],
    ...
    function renderTables(rows) {
      ...
      const productData = rankData(rows, productKey, state.productSort, state.productLimit);
      ...
      renderRank("productTable", productData, "商品", state.productSort, true, "product");
    ...
    function renderRank(id, data, firstTitle, sortBy, scroll = false, kind = "default") {
      const total = data.reduce((sum, d) => sum + (sortBy === "orders" ? d.orders : sortBy === "orderAmount" ? d.orderAmount : d.commission), 0) || 1;
      const rows = data.map((d, i) => {
        const rawName = kind === "product" ? productTitleFromGroup(d) : String(d.key || "");
        const meta = kind === "product" ? productMetaFromGroup(d) : "";
        const name = rawName.length > 72 ? rawName.slice(0, 72) + "..." : rawName;
        const base = sortBy === "orders" ? d.orders : sortBy === "orderAmount" ? d.orderAmount : d.commission;
        const metaHtml = meta ? '<div class="caption name" title="' + esc(meta) + '">' + esc(meta.length > 88 ? meta.slice(0, 88) + "..." : meta) + '</div>' : "";
        return '<tr><td class="rank">' + (i + 1) + '</td><td><div class="name" title="' + esc(rawName) + '">' + esc(name) + '</div>' + metaHtml + '<div class="progress"><span style="width:' + Math.max(2, base / total * 100) + '%"></span></div></td><td class="num">' + money(d.orderAmount) + '</td><td class="num">' + money(d.commission) + '</td><td class="num">' + moneyFmt.format(d.orders) + '</td><td class="num">' + money(d.avgCommission) + '</td></tr>';
      }).join("");
    ...
    function renderDetails(rows) {
      ...
      const trs = recent.map(r => {
        const meta = [r.productCategory, ...(Array.isArray(r.productSellingPoints) ? r.productSellingPoints : []), r.productName || r.productId].filter(Boolean).join("｜");
        return '<tr><td>' + esc(r.orderDate) + '</td><td>' + esc(typeof r.orderHour === "number" ? hourLabel(r.orderHour) : "-") + '</td><td>' + esc(r.site) + '</td><td>' + esc(r.account) + '</td><td><div class="name" title="' + esc(r.productNameCn || r.productName) + '">' + esc(r.productNameCn || r.productName) + '</div><div class="caption name" title="' + esc(meta) + '">' + esc(meta) + '</div></td><td class="num">' + money(r.orderAmount) + '</td><td class="num">' + money(r.actualCommissionCny) + '</td><td class="num">' + esc(r.orderId) + '</td></tr>';
      }).join("");
    ...
    function exportRank(kind) {
      ...
      const data = isAccount
        ...
        : rankData(rows, productKey, sortBy, limit);
      ...
      const bodyRows = data.map(d => [
        isAccount ? d.key : productTitleFromGroup(d),
        ...(isAccount ? [] : [
          d.items?.[0]?.productCategory || "",
          Array.isArray(d.items?.[0]?.productSellingPoints) ? d.items[0].productSellingPoints.join("、") : "",
          productOriginalMetaFromGroup(d),
        ]),
      ...
      downloadExcel(title, isAccount ? headers : ["商品名称-中文", "品类", "卖点", "商品ID / 原商品名", ...headers.slice(1)], bodyRows);
    }
    ...
    function renderCharts(rows) {
      ...
      const productRows = rows.filter(r => productKey(r) === state.productTrend);
    ...
    async function refreshData(projectId = state.projectId) {
      ...
      try {
        ...
        const json = await res.json().catch(() => ({ ok: false, error: "服务端响应异常，请稍后重试" }));
        ...
        setupControls();
        render();
        const cacheText = json.cache?.hit
          ? "快照读取完成：" + moneyFmt.format(payload.source.records) + " 条，后台同步 " + new Date(json.cache.syncedAt || payload.generatedAt).toLocaleString("zh-CN", { hour12: false })
          : "未命中快照，已实时读取：" + moneyFmt.format(payload.source.records) + " 条";
        status.textContent = cacheText;
      } catch (err) {
        ...
        if (/暂无服务器快照/i.test(err.message || "")) {
          status.textContent = err.message;
        } else if (/Data not ready|try again later|timeout|Failed to fetch|NetworkError|AbortError|响应较慢|数据正在计算/i.test(err.message || err.name || "")) {
          status.textContent = "暂无快照且飞书较慢，请点击同步飞书或稍后再试";
        } else {
    ...
    async function syncFeishu(projectId = state.projectId) {
      ...
      btn.disabled = true;
      status.textContent = "正在同步飞书，可能需要几十秒...";
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 90000);
      try {
        const res = await fetch("/api/sync?project=" + encodeURIComponent(projectId), {
          method: "POST",
          headers: { "x-dashboard-code": code },
          signal: controller.signal,
        });
        ...
        setupControls();
        render();
        status.textContent = "飞书同步完成：" + moneyFmt.format(payload.source.records) + " 条记录";
      } catch (err) {
        if (/卡密/.test(err.message || "")) {
          sessionStorage.removeItem(accessKey(projectId));
          sessionStorage.removeItem(codeKey(projectId));
        ...
        if (/Data not ready|try again later|timeout|Failed to fetch|NetworkError|AbortError|响应较慢|数据正在计算/i.test(err.message || err.name || "")) {
          status.textContent = "飞书数据准备中或网络较慢，已保留当前快照";
        } else {
          status.textContent = "同步失败：" + (err.message || "请稍后重试");
    ...
    document.getElementById("refreshBtn").addEventListener("click", () => refreshData(state.projectId));
    document.getElementById("syncBtn").addEventListener("click", () => syncFeishu(state.projectId));
