...
function translationConfig() {
  ...
  return {
    ...
    syncLimit: Number(process.env.PRODUCT_TRANSLATION_SYNC_LIMIT || DEFAULT_SYNC_LIMIT),
    enabled: process.env.PRODUCT_TRANSLATION_ENABLED !== "false",
  };
}

function productCacheKey(row) {
  const base = row.productId || row.productName || row.productNameCn || "";
...
function productIdentity(row) {
  return {
    ...
    feishuNameCn: row.productNameCn || "",
  };
}
...
function fallbackTitle(item) {
  return cleanText(item.feishuNameCn || item.productName || item.productId || "未命名商品", 96);
}
...
async function translateMissingProducts(missing, store, config) {
  ...
  let translated = 0;
  let error = "";
  const limited = missing.slice(0, Math.max(0, config.syncLimit || DEFAULT_SYNC_LIMIT));
...
function seedFallbackProducts(products, store) {
  let seeded = 0;
  for (const item of products) {
    if (store.items[item.key]?.titleCn) continue;
    if (!item.feishuNameCn) continue;
    store.items[item.key] = {
      productId: item.productId,
      productName: item.productName,
      titleCn: fallbackTitle(item),
      category: "",
      sellingPoints: [],
      source: "feishu",
      updatedAt: new Date().toISOString(),
...
async function enrichProductTranslations(rows) {
  ...
  for (const row of rows) {
    if (!row.productId && !row.productName && !row.productNameCn) continue;
  ...
  const seeded = seedFallbackProducts(products, store);
  const config = translationConfig();
  const aiCandidates = products.filter((item) => !store.items[item.key]?.titleCn || store.items[item.key]?.source === "feishu");
  const result = await translateMissingProducts(aiCandidates, store, config);
  if (seeded || result.translated) await writeStore(store);
