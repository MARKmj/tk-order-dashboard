...
function namedCachePath(name) {
  return `${CACHE_PREFIX}/${name}.json`;
}
...
async function readNamedFromBlob(name) {
  if (!hasBlobStore()) return null;
  try {
    const blob = await blobApi.get(namedCachePath(name), { access: "public", useCache: false });
    if (!blob) return null;
    const text = await new Response(blob.stream).text();
    return decodeSnapshot(text);
  } catch (error) {
...
async function writeNamedToBlob(name, data) {
  if (!hasBlobStore()) return null;
  return blobApi.put(namedCachePath(name), encodeSnapshot(data), {
    access: "public",
    contentType: "application/json; charset=utf-8",
    allowOverwrite: true,
  });
...
async function readNamedFromLocal(name) {
  if (!isLocalRuntime()) return null;
  try {
    const file = path.join(LOCAL_CACHE_DIR, `${name}.json`);
    return decodeSnapshot(await fs.readFile(file, "utf8"));
  } catch (error) {
    if (error.code === "ENOENT") return null;
    throw error;
...
async function writeNamedToLocal(name, data) {
  if (!isLocalRuntime()) return null;
  await fs.mkdir(LOCAL_CACHE_DIR, { recursive: true });
  const file = path.join(LOCAL_CACHE_DIR, `${name}.json`);
  await fs.writeFile(file, encodeSnapshot(data));
  return { pathname: file };
}
...
async function readNamedCache(name) {
  return (await readNamedFromBlob(name)) || (await readNamedFromLocal(name));
}
...
async function writeNamedCache(name, data) {
  const stored = await writeNamedToBlob(name, data) || await writeNamedToLocal(name, data);
  if (!stored && process.env.VERCEL) {
    throw new Error("缓存存储未配置，请在 Vercel 添加 BLOB_READ_WRITE_TOKEN");
  }
  return { ...data, storageRef: stored?.url || stored?.pathname || "" };
}
